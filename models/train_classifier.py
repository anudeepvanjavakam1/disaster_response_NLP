...
def load_data(database_filepath):
    """This function reads the table from sqlite database, performs basic cleaning and separates messages and categories

    Args:
        database_filepath (str): file path for database (.db file)

    Returns:
        X (pd.Series): This is a pandas series containing messages. These are the inputs for the classifier.
...
def build_model():
    ...
    pipeline_starting_verb_ext = Pipeline([
            ...
            ('clf', MultiOutputClassifier(AdaBoostClassifier(learning_rate=1, n_estimators=100)))
        ])

    return pipeline_starting_verb_ext
